...
def extract_features(image):
    ...
    # EfficientNetV2 B0 ve renk alanı özelliklerini birleştir
    combined_features = np.concatenate((efficientnet_features, color_features))
...
def main():
    ...
    # ICA ile boyut azaltma
    ica = FastICA(n_components=100, random_state=42)
    train_features = ica.fit_transform(train_features)
    ...
    for fold, (train_indices, test_indices) in enumerate(stratified_kfold.split(train_features, train_labels)):
        ...
        lgbm_classifier.fit(fold_train_features, fold_train_labels)

        fold_predictions = lgbm_classifier.predict(fold_test_features)
        fold_probabilities = lgbm_classifier.predict_proba(fold_test_features)

        fold_accuracy = accuracy_score(fold_test_labels, fold_predictions)
        print(f'LightGBM Test Doğruluğu (Kat {fold + 1}): {fold_accuracy * 100:.2f}%')

        fold_classification_report = classification_report(fold_test_labels, fold_predictions, digits=4)
        print("\nLightGBM Sınıflandırma Raporu (Kat {}):".format(fold + 1))
        print(fold_classification_report)

        fold_mae = mean_absolute_error(fold_test_labels, fold_predictions)
        print(f'LightGBM MAE (Kat {fold + 1}): {fold_mae:.4f}')
    ...
    print("\nGenel LightGBM Sınıflandırma Raporu:")
...
# EfficientNetV2B0 modelini yükle ve özellik çıkarıcı model oluştur
base_model = EfficientNetV2B0(weights='imagenet', include_top=False, pooling='avg')
model = Model(inputs=base_model.input, outputs=base_model.output)

def extract_features(image):
    # EfficientNetV2B0 ile özellik çıkarımı
    image_resized = cv2.resize(image, (224, 224))  # EfficientNetV2B0 için uygun boyut
    image_preprocessed = preprocess_input(image_resized)
    image_preprocessed = np.expand_dims(image_preprocessed, axis=0)
    efficientnet_features = model.predict(image_preprocessed).flatten()
    ...
    # EfficientNetV2B0 ve renk alanı özelliklerini birleştir
    combined_features = np.concatenate((efficientnet_features, color_features))
...
def main():
    ...
    lda = LDA(n_components=None)  # Sınıf sayısından bir eksik bileşene indir
