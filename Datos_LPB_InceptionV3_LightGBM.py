...
def main():
    ...
    for fold, (train_indices, test_indices) in enumerate(stratified_kfold.split(train_features, train_labels)):
        ...
        # LightGBM modeli
        lgb_train = lgb.Dataset(fold_train_features, label=fold_train_labels)
        lgb_eval = lgb.Dataset(fold_test_features, label=fold_test_labels, reference=lgb_train)
        ...
        fold_predictions = np.argmax(gbm.predict(fold_test_features, num_iteration=gbm.best_iteration), axis=1)
        fold_probabilities = gbm.predict(fold_test_features, num_iteration=gbm.best_iteration)
        ...
        for i in range(fold_probabilities.shape[1]):
            fold_precision, fold_recall, _ = precision_recall_curve(fold_test_labels == i, fold_probabilities[:, i])
